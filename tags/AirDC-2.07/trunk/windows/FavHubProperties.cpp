...
LRESULT FavHubProperties::OnInitDialog(UINT, WPARAM, LPARAM, BOOL&)
{
	...
	SetDlgItemText(IDC_FH_RAW, CTSTRING(FAVORITE_HUB_RAW));
	...
	CheckDlgButton(IDC_EXCL_CHECKS, entry->getAutoCDM() ? BST_CHECKED : BST_UNCHECKED); // Excl. from client checking
...
LRESULT FavHubProperties::OnCloseCmd(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	if(wID == IDOK)
	{
		...
		entry->setAutoCDM(IsDlgButtonChecked(IDC_EXCL_CHECKS) == 1); // Excl. from client checking
