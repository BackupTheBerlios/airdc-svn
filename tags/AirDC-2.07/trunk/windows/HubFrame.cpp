...
LRESULT HubFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	autoCDM = (fhe ? fhe->getAutoCDM() : false);
	...
	for(TaskQueue::Iter i = t.begin(); i != t.end(); ++i) {
		...
		UserTask& u = *static_cast<UserTask*>(i->second);
			...
			if(updateUser(u)) {
				...
				if(client->isOp() && !u.onlineUser->getIdentity().isBot() && !u.onlineUser->getIdentity().isHub() && BOOLSETTING(USE_CDM)) {
					int64_t bytesSharedInt64 = u.onlineUser->getIdentity().getBytesShared();
					if(bytesSharedInt64 > 0) {
						tstring detectString;
						bool fakeshare = false;
						...
						for(int i = 0; i < 10; ++i) {
							if(strstr(bytesShared.c_str(), sSameNumbers[i]) != 0) {
								fakeshare = true;
								detectString = Util::formatExactSize(u.onlineUser->getIdentity().getBytesShared()) + _T(" - the share size had too many same numbers in it");
								break;
							}
						}

						double bytesSharedDbl = (double)bytesSharedInt64;
						if ((fmod( bytesSharedDbl, 1048576 )) == 0) {
							fakeshare = true;
							int64_t divided = bytesSharedInt64 / 1048576;
							detectString = Util::formatExactSize(u.onlineUser->getIdentity().getBytesShared()) + _T(" (") + Text::toT(Util::toString(divided)) + _T(" MB) - share size is an integer divided by MB");
						}
						...
						if(fakeshare) {
							...
							//ClientManager::getInstance()->sendRawCommand(u.onlineUser->getUser(), SETTING(FAKESHARE_RAW));
						}
					}

					if(BOOLSETTING(CHECK_NEW_USERS) && autoCDM && BOOLSETTING(USE_CDM)) {
						bool check = true;
						if(BOOLSETTING(PROTECTED_USE_REGEXP)){
							string str1 = SETTING(PROT_USERS);
							string str2 = u.onlineUser->getIdentity().getNick();
							try {
...
/*							PME regexp;
							regexp.Init(Text::utf8ToAcp(SETTING(PROT_USERS)));
							if((regexp.IsValid()) && (regexp.match(Text::utf8ToAcp(u.onlineUser->getIdentity().getNick())))) {
								check = false;
							}*/
						}else{
							if(Wildcard::patternMatch(u.onlineUser->getIdentity().getNick(), SETTING(PROT_USERS), ';', false)) {
								check = false;
							}
						}
						if(check && (u.onlineUser->getIdentity().isTcpActive() || client->isActive())) {
							try {
