...
void SearchFrame::SearchInfo::Download::operator()(SearchInfo* si) {
	try {
		if(si->sr->getType() == SearchResult::TYPE_FILE) {
			...
			QueueManager::getInstance()->addDirectorySearch(si->sr->getFile(), HintedUser(si->sr->getUser(), si->sr->getHubURL()), Text::fromT(tgt),
...
LRESULT SearchFrame::onContextMenu(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	if (reinterpret_cast<HWND>(wParam) == ctrlResults && ctrlResults.GetSelectedCount() > 0) {
		...
		if(ctrlResults.GetSelectedCount() > 0) {
			...
			//copyMenu.AppendMenu(MF_STRING, IDC_COPY_FILEWDIR, CTSTRING(FILEWDIR));
	...
	if ( pos >= 0 ) {
		...
		switch (wID) {
			...
			case IDC_COPY_FILENAME:
				...
				break;
			/* removed so it's more like in copymenu in filelist
			case IDC_COPY_FILEWDIR:
