...
void NetworkPage::fixControls() {
	...
	::EnableWindow(GetDlgItem(IDC_IPUPDATE), direct || upnp || nat || nat_traversal);
	::EnableWindow(GetDlgItem(IDC_GETIP), direct || upnp || nat || nat_traversal);
...
LRESULT NetworkPage::onGetIP(WORD /* wNotifyCode */, WORD /*wID*/, HWND /* hWndCtl */, BOOL& /* bHandled */) {
	...
	c->addListener(this);
	c->downloadFile("http://checkip.dyndns.org/", false);
	return 0;
}
