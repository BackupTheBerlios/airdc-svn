...
/*	if(d->getType() == Transfer::TYPE_FULL_LIST || d->getType() == Transfer::TYPE_PARTIAL_LIST) {
		filelist = true;
	} else {
	filelist = false; 
	}
...
void DownloadManager::failDownload(UserConnection* aSource, const string& reason) {
	...
	if(d) {
		...
		if (d->getType() == Transfer::TYPE_FULL_LIST && reason == STRING(DISCONNECTED)) {
			...
			if(reason == STRING(NO_SLOTS_AVAILABLE) && BOOLSETTING(USE_CDM))
...
/*	} else if ((d->isSet(QueueItem::FLAG_DIRECTORY_DOWNLOAD)) && (d->getType() == Transfer::TYPE_PARTIAL_LIST)){
	...
	// Need to check if it's booth partial and match queue
		...
		return;
	} else if (d->isSet(Download::FLAG_TESTSUR) && BOOLSETTING(USE_CDM)) {
