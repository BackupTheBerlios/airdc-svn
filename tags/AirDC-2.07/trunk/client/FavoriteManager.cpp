...
//temp fix for Adding users depending download speed
//UsersFrame::addUser Freezes the client so Favorite users Frame wont update on User Added this way.
...
void FavoriteManager::save() {
	...
	try {
		...
		for(FavoriteHubEntryList::const_iterator i = favoriteHubs.begin(), iend = favoriteHubs.end(); i != iend; ++i) {
			...
			xml.addChildAttrib("AutoCDM", (*i)->getAutoCDM()); // Excl. from client checking
...
void FavoriteManager::load(SimpleXML& aXml) {
	...
	if(aXml.findChild("Hubs")) {
		...
		while(aXml.findChild("Hub")) {
			...
			e->setAutoCDM(aXml.getBoolChildAttrib("AutoCDM")); // Excl. from client checking
...
/**
 * @file
 * $Id: FavoriteManager.cpp 481 2010-02-11 12:31:20Z bigmuscle $
 */
