...
public:
	...
	FavoriteHubEntry() throw() : connect(true), encoding(Text::systemCharset), chatusersplit(0), favnoPM(false), stealth(false), userliststate(true), mode(0), ip(Util::emptyString), hideShare(false), autoCDM(false), searchInterval(SETTING(MINIMUM_SEARCH_INTERVAL))  { }
	FavoriteHubEntry(const HubEntry& rhs) throw() : name(rhs.getName()), server(rhs.getServer()), encoding(Text::systemCharset), searchInterval(SETTING(MINIMUM_SEARCH_INTERVAL)),
		description(rhs.getDescription()), connect(true), chatusersplit(0), favnoPM(false), stealth(false), userliststate(true), mode(0), ip(Util::emptyString), hideShare(false), autoCDM(false) { }
	FavoriteHubEntry(const FavoriteHubEntry& rhs) throw() : userdescription(rhs.userdescription), name(rhs.getName()), 
		...
		nick(rhs.nick), chatusersplit(rhs.chatusersplit), favnoPM(rhs.favnoPM), stealth(rhs.stealth), searchInterval(rhs.searchInterval),
		userliststate(rhs.userliststate), mode(rhs.mode), ip(rhs.ip), hideShare(rhs.hideShare), autoCDM(rhs.autoCDM), encoding(rhs.getEncoding()),
	...
	GETSET(bool, autoCDM, AutoCDM); // Excl. from client checking
