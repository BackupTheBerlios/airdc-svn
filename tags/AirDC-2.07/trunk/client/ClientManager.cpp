...
void ClientManager::fileListDisconnected(const UserPtr& p) {
	...
	Client* c = NULL;
	{
		...
		if(i != onlineUsers.end()) {
			...
			if(fileListDisconnects == SETTING(ACCEPTED_DISCONNECTS) && BOOLSETTING(USE_CDM)) {
				c = &ou.getClient();
...
void ClientManager::connectionTimeout(const UserPtr& p) {
	...
	Client* c = NULL;
	{
		...
		if(i != onlineUsers.end()) {
			...
			if(connectionTimeouts == SETTING(ACCEPTED_TIMEOUTS) && BOOLSETTING(USE_CDM)) {
				c = &ou.getClient();
...
void ClientManager::checkCheating(const UserPtr& p, DirectoryListing* dl) {
	...
	OnlineUserPtr ou = NULL;
	{
		...
		if(isFakeSharing && BOOLSETTING(USE_CDM)) {
...
void ClientManager::setParams(const UserPtr& p, const StringMap& params) {
	OnlineUser* ou = NULL;
	{
		Lock l(cs);
		OnlineIterC i = onlineUsers.find(const_cast<CID*>(&p->getCID()));
		...
		for(StringMap::const_iterator i = params.begin(); i != params.end(); i++) {
			ou->getIdentity().set(i->first.c_str(), i->second);
		}
	}
}
...
/**
 * @file
 * $Id: ClientManager.cpp 481 2010-02-11 12:31:20Z bigmuscle $
 */
