...
HTREEITEM QueueFrame::createDir(TVINSERTSTRUCT& tvi, string&& dir, const BundlePtr& aBundle, HTREEITEM parent, bool subDir /*false*/) {
...
HTREEITEM QueueFrame::createSplitDir(TVINSERTSTRUCT& tvi, string&& dir, HTREEITEM parent, DirItemInfo* dii, bool subDir /*false*/) {
	/* Bundles */
	...
	if (reinterpret_cast<HWND>(wParam) == ctrlQueue && ctrlQueue.GetSelectedCount() > 0) { 
		...
		if(ctrlQueue.GetSelectedCount() > 0) { 
			...
			if(ctrlQueue.GetSelectedCount() == 1) {
				...
				if(ii && !ii->isSet(QueueItem::FLAG_USER_LIST)) {
					WinUtil::appendPreviewMenu(fileMenu, ii->getTarget());
				}
		...
		if (mainBundle) {
			...
			for(auto& bs: badBundleSources) {
				auto u = bs.user;
				readdMenu->appendItem(formatUser(bs), [=] { QueueManager::getInstance()->readdBundleSource(b, u); });
			}
		}
		/* Submenus end */
